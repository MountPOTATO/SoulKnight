...
bool Mummy::init() {
	Sprite* sprite = Sprite::create("Enemy\\Mummy.png");
	this->bindSprite(sprite);
	setAnchorPoint(Point(0.5f, 0.5f));
	setSpeed(MUMMY_SPEED);
	setHP(MUMMY_HP);
	isAiMoving = false;
	m_monsterID = 3;
	...
	remainChargingDistance = MUMMY_MAX_CHARGES_ROUND;
	return true;
}
...
void Mummy::update(float delta) {
	...
	if (remainChargingDistance > 0 && isChargeBlocked == false) {
		...
		float x_move = x_distance / xy_distance * MUMMY_CHARGE_SPEED;
		float y_move = y_distance / xy_distance * MUMMY_CHARGE_SPEED;//按比例获取x,y的移动距离
		...
		if (max_twist > 0) {
			...
			if (remainMovingDistance > 0) {
			   ...
			   //进行一次移动
				...
				{
				case left:
					dstPos = curPos + Point(-MUMMY_SPEED, 0);
					break;
				case upLeft:
					dstPos = curPos + Point(-MUMMY_SPEED / 1.414, MUMMY_SPEED / 1.414);
					break;
				case up:
					dstPos = curPos + Point(0, MUMMY_SPEED);
					break;
				case upRight:
					dstPos = curPos + Point(MUMMY_SPEED / 1.414, MUMMY_SPEED / 1.414);
					break;
			...
			if (remainMovingDistance == 0) {
				...
				remainMovingDistance = MUMMY_MAX_MOVES_ROUND;
			}
		}
		if (max_twist == 0) {
...
bool Mummy::isPosBlocked(Point dstPos) {
...
void Mummy::calDistance() {
	...
	nextMovingDistance = (int)(MUMMY_MAX_MOVES_ROUND * CCRANDOM_0_1() + 1);//生成介于1-3的整数
